...
/** Endpoint 1: Get all yip data or yip data matching a given search term, return all yips or all ids of yips who matches the given search term. */
app.get('/yipper/yips', async (req, res) => {
  try {
    ...
    await db.close();
    res.type('json').status(200).send({ yips: rows });
  } catch (err) {
    res.type('txt').status(500).send(SERVER_ERROR_MESSAGE);
  }
});
...
app.get('/yipper/user/:user', async (req, res) => {
  try {
    ...
    if (rows.length === 0) {
      ...
      res.type('json').status(200).send(rows);
    }
  } catch (err) {
    res.type('txt').status(500).send(SERVER_ERROR_MESSAGE);
  }
...
app.post('/yipper/likes', async (req, res) => {
  if (!req.body.id) {
    ...
    try {
      ...
      if (!row) {
        await db.close();
        res.type('txt').status(400).send(`Yikes. ID does not exist.`);
      } else {
  ...
  if (!(name && yip && hashtag)) {
    ...
    try {
      ...
      if (!result?.id) {
        ...
        res.type('json').status(200).send(result3);
      }
    } catch (err) {
      res.type('txt').status(500).send(SERVER_ERROR_MESSAGE);
    }
